...
from ur5_reaching import UR_REACHING
...
class PolicyNetwork(nn.Module):
    ...
    def evaluate(self, state, deterministic, eval_noise_scale, epsilon=1e-6):
        ...
        normal = Normal(0, 1)
        z      = normal.sample() 
        action_0 = torch.tanh(mean.to(device) + std*z.to(device)) # TanhNormal distribution as actions; reparameterization trick
        action_range = self.action_range.to(device)
        action = action_range*mean if deterministic else action_range*action_0
        log_prob = Normal(mean.cpu(), std.cpu()).log_prob(mean.cpu()+ std.cpu()*z.cpu()) - torch.log(1. - action_0.pow(2).cpu() + epsilon) -  np.log(action_range.cpu())
...
action_range = torch.tensor(action_range, dtype = torch.float32, device = device)#torch.device('cpu'))
...
max_episodes  = 2000 #10000
max_steps   = 50 #20
frame_idx   = 0
batch_size  = 750#300#150
...
DETERMINISTIC=False#True  # DDPG: deterministic policy gradient
explore_noise_scale = 0.01  # 0.5 noise is required for Pendulum-v0, 0.1 noise for HalfCheetah-v2
eval_noise_scale = 0.01
reward_scale = 1.
rewards     = []
model_path = './model/td3_ur_reaching'
...
if __name__ == '__main__':
    ...
    if train:
        ...
        writer = SummaryWriter(comment="TD3_UR_REACHING")
        episode_reward = 0
        rewards = []
        total_timesteps = 0
        ...
        for eps in range(max_episodes):
            ...
            writer.add_scalar("episode_reward", episode_reward, eps)
            # writer.add_scalar("dist_to_goal", info, eps)
            writer.add_scalar("Difference in x", info[0], eps)
            writer.add_scalar("Difference in y", info[1], eps)
            writer.add_scalar("Difference in z", info[2], eps)
            
            if eps % 2 == 0 and eps>0:
